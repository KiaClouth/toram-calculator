...
export default function MonserPageClient(props: Props) {
  ...
  const [defaultCrystalList, setDefaultCrystalList] = useState(props.crystalList);
  ...
  const {
    ...
    setCrystalList,
    crystalDialogState,
    setCrystalDialogState,
    setCrystalFormState,
    filterState,
    setFilterState,
  } = useStore((state) => state.crystalPage);
  const { crystal, setCrystal } = useStore((state) => state);

  // 搜索框行为函数
  const handleSearchFilterChange = (value: string) => {
    const currentList = defaultCrystalList;
    if (value === "" || value === null) {
    ...
    currentList.forEach((crystal) => {
      let filter = false;
      for (const attr in crystal) {
        if (!crystalHiddenData.includes(attr as keyof Crystal)) {
          const crystalAttr = crystal[attr as keyof Crystal]?.toString();
          if (crystalAttr?.match(value)?.input !== undefined) {
            filter = true;
          }
        }
      }
      filter ? newCrystalList.push(crystal) : null;
    });
    setCrystalList(newCrystalList);
  };

  // 弹出层同名怪物列表
  const [sameNameCrystalList, setSameNameCrystalList] = useState<Crystal[]>([]);
  const compusteSameNameCrystalList = (crystal: Crystal, crystalList: Crystal[]) => {
    const list: Crystal[] = [];
    crystalList.forEach((c) => {
      c.name === crystal.name && list.push(c);
    });
    return list.sort((crystalA, crystalB) => {
      const dateA = new Date(crystalA.updatedAt);
      const dateB = new Date(crystalB.updatedAt);
      return dateA.getTime() - dateB.getTime();
    });
  };

  // 定义不需要展示的列
  const crystalHiddenData: Array<keyof Crystal> = ["id"];

  // 列定义
  const columns = React.useMemo<ColumnDef<Crystal>[]>(
    () => [
      {
        accessorKey: "id",
        header: () => dictionary.db.models.crystal.id,
        cell: (info) => info.getValue(),
        size: 200,
      },
      {
        accessorKey: "name",
        header: () => dictionary.db.models.crystal.name,
        cell: (info) => info.getValue(),
        size: 180,
      },
      {
        accessorKey: "modifiersList",
        header: () => dictionary.db.models.crystal.modifiersList.modifiers,
        cell: (info) => {
          const modifiers = info.getValue<ModifiersList>().modifiers;
          const formulas = modifiers.map((mod) => mod.formula);
          return formulas.join("  |  ");
        },
        size: 620,
      },
      {
        accessorKey: "crystalType",
        header: () => dictionary.db.models.crystal.crystalType,
        cell: (info) => dictionary.db.enums.CrystalType[info.getValue<$Enums.CrystalType>()],
        size: 120,
      },
      {
        accessorKey: "front",
        header: () => dictionary.db.models.crystal.front,
        cell: (info) => info.getValue(),
        size: 140,
      },
      {
        accessorKey: "updatedAt",
        header: dictionary.db.models.crystal.updatedAt,
    ...
    [
      ...
      dictionary.db.models.crystal.id,
      dictionary.db.models.crystal.name,
  ...
  // 表头固定
  const getCommonPinningStyles = (column: Column<Crystal>): CSSProperties => {
  ...
  const handleMouseDown = (id: string, e: React.MouseEvent) => {
    ...
    const handleMouseUp = () => {
      ...
      if (!isDragging) {
        console.log(id);
        const targetCrystal = crystalList.find((crystal) => crystal.id === id);
        if (targetCrystal) {
          setCrystal(targetCrystal);
          setSameNameCrystalList(compusteSameNameCrystalList(targetCrystal, crystalList));
          setCrystalDialogState(true);
  ...
  useEffect(() => {
    console.log("--Crystal Client Render");
    setCrystalList(defaultCrystalList);
    // u键监听
    const handleUKeyPress = (e: KeyboardEvent) => {
      if (e.key === "u") {
        setCrystal(defaultCrystal);
        setSameNameCrystalList([]);
        setCrystalDialogState(true);
        setCrystalFormState("CREATE");
      }
    };
    document.addEventListener("keydown", handleUKeyPress);
    return () => {
      console.log("--Crystal Client Unmount");
      document.removeEventListener("keydown", handleUKeyPress);
    };
  }, [defaultCrystalList, setCrystal, setCrystalDialogState, setCrystalFormState, setCrystalList]);
  ...
  return (
    <main className="flex flex-col lg:w-[calc(100dvw-96px)] lg:flex-row">
          ...
          <div className="module flex flex-col gap-3">
            ...
            <div className="content flex flex-wrap gap-2 ">
              ...
              {table.getAllLeafColumns().map((column) => {
                if (crystalHiddenData.includes(column.id as keyof Crystal)) {
                  // 默认隐藏的数据
                  return;
                }
                return (
                  ...
                  >
                    {typeof dictionary.db.models.crystal[column.id as keyof Crystal]}
                  </Button>
                );
              })}
            </div>
          ...
          <div className="Title sticky left-0 mt-3 flex flex-col gap-9 py-5 lg:pt-20">
            <div className="Row flex flex-col items-center justify-between gap-10 lg:flex-row lg:justify-start lg:gap-4">
              <h1 className="Text text-left text-3xl lg:bg-transparent lg:text-4xl">
                {dictionary.ui.crystal.pageTitle}
              </h1>
                      ...
                      {table.getAllLeafColumns().map((column) => {
                        ...
                        return (
                          <Button
                            ...
                            onClick={column.getToggleVisibilityHandler()}
                          >
                            {_.isString(dictionary.db.models.crystal[column.id as keyof Crystal])
                              ? (dictionary.db.models.crystal[column.id as keyof Crystal] as string)
                              : column.id}
                          </Button>
                        );
                      })}
                ...
                return (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => {
                      ...
                      switch (cell.column.id as Exclude<keyof Crystal, keyof typeof crystalHiddenData>) {
                        case "name":
                          return (
                            <td
                              key={cell.id}
      ...
      <Dialog state={crystalDialogState} setState={setCrystalDialogState}>
        {crystalDialogState && (
          <div className="Content flex w-full flex-col overflow-y-auto lg:flex-row 2xl:w-[1536px]">
            {sameNameCrystalList.length > 1 && (
              <div className="SameNameCrystalList flow-row flex flex-none basis-[8%] gap-1 overflow-x-auto overflow-y-hidden border-r-1.5 border-brand-color-1st p-3 lg:w-60 lg:flex-col lg:overflow-y-auto lg:overflow-x-hidden">
                {sameNameCrystalList.map((currentCrystal) => {
                  const order = sameNameCrystalList.indexOf(currentCrystal) + 1;
                  return (
                    <Button
                      key={"SameNameCrystalId" + currentCrystal.id}
                      level="tertiary"
                      onClick={() => {
                        setCrystal(currentCrystal);
                      }}
                      active={currentCrystal.id === crystal.id}
                      className="SameNameCrystal flex h-full basis-1/4 flex-col rounded-sm lg:h-auto lg:w-full lg:basis-auto"
                    >
                      ...
                      <span className="hidden text-left text-sm lg:block">
                        {currentCrystal.updatedAt.toLocaleString()}
                      </span>
                    </Button>
                  );
                })}
            ...
            <CrystalForm
              ...
              session={session}
              defaultCrystalList={defaultCrystalList}
              setDefaultCrystalList={setDefaultCrystalList}
            />
          </div>
        )}
      </Dialog>
