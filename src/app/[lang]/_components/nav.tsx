...
export default function Nav(props: {
  ...
  // tw(`Nav border-t-1 z-30 flex w-dvw flex-shrink-0 overflow-x-auto border-transition-color-20 backdrop-blur 
  // lg:[
  ...
  return (
    <div className="Nav border-t-1 z-30 flex w-dvw flex-shrink-0 overflow-x-auto border-transition-color-20 backdrop-blur 
    lg:fixed lg:left-0 lg:h-dvh lg:w-24 lg:-translate-x-3/4 lg:flex-col lg:gap-10 lg:border-none lg:bg-transition-color-8 lg:py-5 lg:opacity-0 lg:hover:translate-x-0 lg:hover:opacity-100 lg:dark:left-[3dvw] lg:dark:top-1/2 lg:dark:h-[90dvh] lg:dark:-translate-y-1/2 lg:dark:translate-x-0 lg:dark:opacity-100">
      <div>
        <Link
          href={"/"}
      ...
      <div className="NavBtnList flex flex-1 items-center lg:flex-col lg:gap-4 lg:overflow-y-auto">
        {NavBtnConfig.map((group) => {
          return (
            <div
              key={group.name}
              className={`${group.name} BtnGroup flex flex-initial items-center lg:flex-col lg:gap-4`}
            >
              {group.children.map((btn) => {
                return (
                  <Link
                    ...
                    href={btn.url}
                    tabIndex={0}
                    className={`NavBtn btn-${btn.name} group flex flex-shrink-0 flex-col items-center gap-0.5 px-1 py-2 lg:gap-1 lg:p-0`}
                  >
